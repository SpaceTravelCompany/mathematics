...
> 학습 순서 49 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 미분방정식은 상태의 시간 변화를 정하고, 고유값은 각 방향의 변화율을 나눠 보는 도구다.

$x'=-x$의 해는 $x(t)=x(0)e^{-t}$다. 처음 값이 조금 양수든 음수든 0으로 가까워진다. $x=0$은 변화율이 0인 **평형점**이고, 주변 상태들이 되돌아오므로 점근적으로 안정하다. $x'=x$에서는 작은 차이가 $e^t$배 커져 0에서 멀어지므로 불안정하다.

두 성분의 계 $x'=-x$, $y'=y$에서는 가로 방향은 되돌아오고 세로 방향은 멀어진다. 평형점 전체는 불안정한 안장 형태다. 일반 비선형 계에서도 평형점 근처의 야코비안으로 이런 선형 근사를 만든다.

연속시간 $x'=Ax$는 고유값의 **실수부가 음수**이면 감쇠한다. 이산시간 $x_{n+1}=Ax_n$는 고유값의 **절댓값이 1보다 작아야** 감쇠한다. 시간에 따른 $e^{\lambda t}$와 반복 곱 $\lambda^n$이 다르기 때문이다.

**본문으로 이어 읽기:** 고유값의 실수부가 0인 경계 상황은 선형 근사만으로 결론을 내리지 못할 수 있다. 카오스는 결정론적인 규칙 아래 초기 차이가 민감하게 커지는 현상이다. 무작위 잡음을 넣었다는 뜻과는 다르며, 모든 비선형 계가 카오스를 보이는 것도 아니다.
...
[← 이전: 상미분방정식 기초](ode-basics.html) · [다음: 변분법 →](calculus-of-variations.html)
