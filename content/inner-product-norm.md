...
> 학습 순서 18 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 평면벡터에서 같은 성분끼리 곱해 더한 것이 내적이다.

$u=(3,4)$이면 $u\cdot u=9+16=25$이고 길이 $\|u\|=5$다. 노름 기호 $\|\ \|$는 여기서 벡터의 길이를 뜻한다. $u-v$는 두 점 사이 이동이므로 $\|u-v\|$는 두 점 사이 거리다.

$u=(2,0)$, $v=(10,0)$은 같은 방향이지만 내적이 20이다. $v=(1,0)$이면 내적은 2다. 내적에는 방향뿐 아니라 길이도 들어 있으므로 방향만 비교하려면 $\frac{u\cdot v}{\|u\|\|v\|}$로 나누어야 한다. 이것이 코사인 유사도다. 두 예 모두 값이 1이다.

내적은 ‘한 벡터의 다른 방향으로의 부호 있는 그림자 길이 × 그 방향 벡터의 길이’다. 방향 벡터가 단위벡터일 때에만 내적 자체가 그림자 길이가 된다. 영벡터는 분모가 0이므로 코사인 유사도를 정의할 수 없다.

**증명으로 이어 읽기:** 코시-슈바르츠 부등식 $|u\cdot v|\le\|u\|\|v\|$는 그림자가 원래 길이보다 길 수 없다는 관찰을 일반화한다. 추상 내적에서도 제곱 길이 $\|u-tv\|^2\ge0$를 이용해 같은 결론을 얻는다.

---

## 직관적 설명

**내적(inner product, dot product)**은 두 벡터가 얼마나 같은 방향을 향하는지를 하나의 숫자로 요약한다. 한 벡터의 다른 벡터 방향으로의 부호 있는 투영 길이에, 그 다른 벡터의 길이를 곱한 값이다. 따라서 내적에는 두 벡터의 길이도 반영된다. 0이 아닌 두 벡터의 내적이 0이면 수직이다.
...
- **[평면벡터 기초](plane-vectors.html)** : 2차원 벡터의 내적과 기하학적 의미를 먼저 학습한다. 이 문서는 내적을 추상 벡터공간으로 일반화한다.
- **[직교성·직교투영·그람-슈미트](orthogonality.html)** : 내적이 0인 벡터들의 관계와 벡터 분해로 확장한다.
- **[SVD](svd.html)** : 특이값 분해는 내적공간의 기하를 행렬분해에 적용한 정점이다.

---

[← 이전: 벡터공간 추상화](vector-space-abstraction.html) · [다음: 직교성·직교투영·그람-슈미트 →](orthogonality.html)
