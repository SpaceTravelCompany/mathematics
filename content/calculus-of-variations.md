...
> 학습 순서 50 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 최적화는 입력을 바꾸어 출력값을 줄이는 일이다. 여기서는 입력이 점 하나가 아니라 경로 전체다.

시작과 끝을 잇는 경로 $y(x)$에 비용 $J[y]=\int_0^1(y'(x))^2dx$를 매기자. $J$는 함수 하나를 받아 숫자를 내놓으므로 **범함수**라고 부른다. 경로를 조금 바꾸면 비용이 얼마나 바뀌는지가 변분법의 질문이다.

$y(0)=0,y(1)=1$이면 $\int_0^1y' dx=1$이다. 따라서

$$\int_0^1(y'-1)^2dx=\int_0^1(y')^2dx-2\cdot1+1=J[y]-1\ge0.$$

즉 $J[y]\ge1$이고 $y'=1$, 곧 $y=x$에서 최솟값 1을 얻는다. ‘경로 전체를 고른다’는 말을 실제 적분 계산으로 풀어 본 예다.

일반 비용 $J[y]=\int L(x,y,y')dx$에서는 작은 경로 변화 $y+\epsilon\eta$를 넣는다. $\eta$는 변형 모양, $\epsilon$은 그 크기이며 끝점을 고정하므로 $\eta$는 양 끝에서 0이다. 이때 $\epsilon=0$에서 비용의 미분이 0이라는 조건을 부분적분으로 정리한 것이 오일러-라그랑주 방정식이다.

**증명으로 이어 읽기:** $\partial L/\partial y-d(\partial L/\partial y')/dx=0$은 최적 경로의 후보 조건이다. 보통 미분에서처럼 이것만으로 최솟값인지 확정하지 않는다. 여기서 $y$와 $y'$는 $L$의 입력 자리로는 별도로 미분하지만 실제 경로에서는 도함수 관계로 연결된다.
...
[← 이전: 미분방정식과 동역학](dynamical-systems.html) · [다음: 경우의 수·순열·조합 →](counting.html)
