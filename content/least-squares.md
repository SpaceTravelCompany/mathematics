...
> 학습 순서 20 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 정확히 닿지 못하는 목표에는 가장 가까운 점을 찾고, 그때 오차는 가능한 이동 방향과 수직이다.

두 관측값 1과 3을 숫자 하나 $c$로 대표하고 싶다고 하자. 오차 제곱합은 $(c-1)^2+(c-3)^2$다. 펼쳐서 다시 묶으면

$$(c-1)^2+(c-3)^2=2(c-2)^2+2.$$

제곱은 0 이상이므로 $c=2$일 때 최솟값 2다. 미분을 아직 몰라도 이 계산으로 ‘오차의 제곱합을 최소화한다’는 뜻을 이해할 수 있다. 오차를 그냥 더하면 양수와 음수가 상쇄되어 틀린 답이 좋아 보이므로 제곱을 쓴다.

행렬로는 $A=(1,1)^T$, $b=(1,3)^T$이며 $Ac=(c,c)^T$다. 최적점 $(2,2)$에서 잔차 $b-Ac=(-1,1)$은 이동 방향 $(1,1)$과 내적이 0이다. 여러 열이 있어도 각 열과 잔차의 내적이 0이어야 하므로 $A^T(b-Ax)=0$, 즉 $A^TAx=A^Tb$를 얻는다.

**증명으로 이어 읽기:** $A^TA$의 역행렬을 쓰려면 $A$의 열들이 독립이어야 한다. 그렇지 않아도 최소제곱 문제는 풀 수 있지만 계수의 해가 여러 개일 수 있다. 아래 미분을 이용한 유도는 [편도함수](partial-derivatives.html)에서 배우는 같은 최적화 조건의 다른 표현이다.
...
[← 이전: 직교성·직교투영·그람-슈미트](orthogonality.html) · [다음: 고유값·고유벡터 →](eigenvalues.html)
