...
> 학습 순서 65 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 최소제곱법은 관측과 예측의 차이를 제곱해 더한 값을 줄이는 방법이다.

자료 $(x,y)=(0,1),(1,3),(2,5)$는 $\hat y=1+2x$에 정확히 놓인다. 절편 1은 $x=0$에서의 예측, 기울기 2는 $x$가 1 늘 때의 예측 변화다. 실제 자료가 $(0,1),(1,4),(2,5)$처럼 흔들리면 모든 점을 지나는 직선이 없으므로 잔차 제곱합이 작은 직선을 고른다.

모형 $Y=\beta_0+\beta_1X+\epsilon$에서 오차 $\epsilon$은 우리가 모르는 실제 흔들림이고, 잔차 $e_i=y_i-\hat y_i$는 데이터를 맞춘 뒤 계산한 차이다. 둘을 같은 기호로 취급하면 가정과 계산 결과가 뒤섞인다.

선형회귀의 ‘선형’은 계수에 대해 선형이라는 뜻이다. $\beta_0+\beta_1x+\beta_2x^2$도 계수들에는 선형이므로 선형회귀로 다룰 수 있다. 자료 밖으로 크게 외삽하면 관측 구간의 관계가 유지된다는 보장이 없다.

**증명으로 이어 읽기:** 가우스-마르코프 정리는 설명변수와 오차에 대한 조건, 같은 오차분산, 오차 간 비상관성 등 아래에서 OLS가 선형 불편 추정량 중 최소분산임을 말한다. 정규성은 그 결론 자체에 필수는 아니지만 작은 표본의 정확한 t검정에는 추가 가정으로 쓰인다. 회귀계수만으로 인과효과가 자동으로 정해지지는 않는다.
...
[← 이전: 가설검정·p-value·신뢰구간](hypothesis-testing.html) · [다음: 확률 행렬·마르코프 체인 →](markov-chains.html)
