...
> 학습 순서 25 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 직교행렬은 길이를 유지하며 좌표를 바꾸고, 대각행렬은 축별로 늘리거나 줄인다.

행렬 $A=\left(\begin{smallmatrix}3&0\\0&1\end{smallmatrix}\right)$는 단위원을 가로 반지름 3, 세로 반지름 1인 타원으로 보낸다. 여기의 늘어나는 비율 3과 1이 특이값이다. 더 일반적인 행렬에서도 입력과 출력의 축을 잘 고르면 이처럼 축별 늘리기로 설명할 수 있다.

$A=U\Sigma V^T$를 오른쪽부터 읽자. $V^T$로 입력의 기준 축을 맞추고, $\Sigma$로 각 축을 특이값만큼 늘리고, $U$로 출력의 축을 맞춘다. 직교변환에는 회전뿐 아니라 반사도 포함된다. 직사각형 행렬이면 $\Sigma$도 직사각형이어서 남는 자리를 0으로 보내거나 출력 성분을 더한다.

고유값 분해는 입력과 출력에 같은 기저를 쓰려 하지만, SVD는 두 기저를 따로 허용한다. 그래서 실수 직사각형 행렬에도 항상 가능하다. 특이값은 0 이상이며 $A^TA$의 고유값에 제곱근을 취한 값이다. $A^TA$가 양반정치이므로 이 제곱근을 실수로 취할 수 있다.

**본문으로 이어 읽기:** 작은 특이값 방향은 출력에 기여하는 길이가 작다. 이를 버리면 정보 일부를 잃는 대신 적은 재료로 행렬을 근사한다. ‘최적 저계수 근사’는 정해진 rank 안에서 프로베니우스 노름 또는 스펙트럴 노름으로 잰 오차가 가장 작다는 뜻이다.
...
[← 이전: 양정치 행렬](positive-definite.html) · [다음: 극한·연속·도함수 →](limits-derivatives.html)
