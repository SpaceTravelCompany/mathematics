...
> 학습 순서 54 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 확률분포는 각 결과에 얼마의 비중을 줄지 정한 것이다.

$X$가 0 또는 2를 각각 $1/2$ 확률로 가지면 기댓값은 $0\times1/2+2\times1/2=1$이다. 실제 가능한 값에 1이 없어도 평균은 1일 수 있다. $\mathbb E[X]$는 가장 흔한 결과가 아니라 확률로 가중한 평균이다.

평균에서의 차이는 -1과 1이므로 그냥 평균하면 0이다. 퍼짐을 재려면 제곱해 평균하여 $\operatorname{Var}(X)=1$을 얻는다. 표준편차는 그 제곱근인 1이다. $\mathbb E[X^2]=2$에서 $(\mathbb E[X])^2=1$을 빼도 같은 분산을 얻는다.

$Y=3X$이면 평균은 3배, 평균에서의 차이도 3배다. 따라서 분산은 $3^2=9$배가 된다. 반면 $X+10$은 전체 위치만 옮기므로 분산이 그대로다.

**본문으로 이어 읽기:** 공분산은 두 변수의 ‘평균에서의 차이’를 곱해 평균한 값이다. 함께 커지면 양수, 반대로 움직이면 음수가 되기 쉽다. 상관계수는 공분산을 두 표준편차로 나누어 단위를 없앤 값이다. 분산이 0이면 이 나눗셈이 정의되지 않는다. 상관 0은 선형 관계가 없다는 뜻이지 모든 의존성이 없다는 뜻은 아니다.
...
[← 이전: 확률변수·PMF/PDF](random-variables.html) · [다음: 주요 분포 →](distributions.html)
