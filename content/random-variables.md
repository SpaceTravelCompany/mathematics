...
> 학습 순서 53 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 함수는 입력마다 출력 하나를 붙이는 규칙이고, 확률은 결과의 모임에 부여한다.

동전을 두 번 던진 결과 HH, HT, TH, TT에 ‘앞면 개수’를 붙이면 각각 $2,1,1,0$이다. 이 숫자 붙이기 규칙이 확률변수 $X$다. 공정한 독립 동전이면 $P(X=0)=1/4$, $P(X=1)=1/2$, $P(X=2)=1/4$다. 숫자 1로 가는 결과가 두 개라서 확률이 두 배다.

이산형의 PMF는 각 값의 확률을 적은 표다. 연속형의 PDF는 **확률밀도**로, 높이가 아니라 구간 아래 넓이가 확률이다. $[0,2]$에서 균등하면 밀도는 $1/2$이고, $[0.5,1]$에 들어갈 확률은 폭 $0.5$에 높이 $0.5$를 곱한 $0.25$다.

연속형에서 정확한 한 점은 폭이 0이므로 보통 $P(X=x)=0$이다. 밀도는 1보다 클 수도 있다. $[0,0.5]$에서 균등하면 높이는 2지만 전체 넓이는 여전히 1이다.

**본문으로 이어 읽기:** CDF $F(x)=P(X\le x)$는 기준값까지 누적한 확률이다. 이산형·연속형 모두에 쓰며, 밀도가 있는 연속형에서는 $F(x)=\int_{-\infty}^x f(t)dt$다. 대문자 $X$는 규칙 또는 무작위 값, 소문자 $x$는 그 값의 후보나 관측값을 뜻한다.
...
**예제 6 (CDF에서 PDF 도출):** 확률변수 $X$의 CDF가 $F(x) = 1 - e^{-2x}$ ($x \geq 0$)일 때 PDF를 구하라.

**풀이:** $f(x) = F'(x) = 2e^{-2x}$ ($x \geq 0$), $x < 0$에서는 $f(x) = 0$이다.

**예제 7 (분위수 계산):** $X$의 CDF가 $F(x) = x^2$ ($0 \leq x \leq 1$)일 때 중앙값을 구하라.

**풀이:** $F^{-1}(p) = \sqrt{p}$이므로 중앙값 $m = F^{-1}(0.5) = \sqrt{0.5} \approx 0.707$.

**예제 8 (결합분포에서 주변분포):** 결합 PDF $f_{X,Y}(x,y) = x + y$ ($0 \leq x, y \leq 1$)에서 $X$의 주변 PDF를 구하라.

**풀이:**
$$f_X(x) = \int_0^1 (x + y)\,dy = \left[xy + \frac{y^2}{2}\right]_0^1 = x + \frac{1}{2}, \quad 0 \leq x \leq 1$$

**예제 9 (PDF의 정규화 상수 결정):** $f(x) = c x^2$ ($0 \leq x \leq 1$)가 PDF가 되도록 상수 $c$를 결정하라.
...
**예제 10 (CDF로 확률 계산):** $X$의 CDF $F(x) = 1 - (1+x)e^{-x}$ ($x \geq 0$)일 때 $P(1 < X < 2)$를 구하라.
...
**예제 11 (이산 확률변수의 CDF):** $X$의 PMF가 $p(0) = 0.2$, $p(1) = 0.5$, $p(2) = 0.3$일 때 CDF $F(x)$를 구하고 그래프의 형태를 설명하라.
...
[← 이전: 확률·조건부확률·베이즈 정리](conditional-bayes.html) · [다음: 기댓값·분산·공분산·상관계수 →](expectation-variance.html)
