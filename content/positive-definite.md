...
> 학습 순서 24 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 대칭행렬의 이차식은 고유벡터 좌표에서 제곱들의 가중합으로 바뀐다.

$A=\left(\begin{smallmatrix}2&0\\0&3\end{smallmatrix}\right)$이면 $x^TAx=2x_1^2+3x_2^2$다. $x\ne0$이면 적어도 하나의 제곱이 양수이므로 결과도 양수다. 이것이 **양정치**다. 행렬의 모든 성분이 양수라는 뜻은 아니다. 위 행렬에는 0도 있다.

$B=\left(\begin{smallmatrix}2&0\\0&0\end{smallmatrix}\right)$는 $x^TBx=2x_1^2\ge0$이지만 $(0,1)$에서는 0이 된다. 이런 경우는 양반정치다. $C=\operatorname{diag}(1,-1)$은 $(1,0)$에서 양수, $(0,1)$에서 음수이므로 부정부호다.

$2x_1^2+3x_2^2=1$의 축 길이는 $1/\sqrt2$, $1/\sqrt3$이다. 고유값이 클수록 같은 값 1에 더 빨리 도달하므로 해당 축은 짧다. 뒤의 미분에서는 이러한 이차식을 국소적인 곡면의 모양으로 사용한다.

**증명으로 이어 읽기:** $x^TA^TAx=(Ax)^T(Ax)=\|Ax\|^2\ge0$이므로 $A^TA$는 언제나 양반정치다. 0이 아닌 $x$가 $Ax=0$을 만족하지 않을 때에만 양정치가 된다. 이 조건은 앞에서 배운 열의 일차독립과 같다.
...
**선도 주행렬식 (leading principal minor, 아래에서는 주소행렬식):** $A$의 $k$번째 주소행렬식 $\Delta_k$는 $A$의 $1, \ldots, k$번째 행과 열로 이루어진 $k \times k$ 부분행렬의 행렬식이다.
...
1. $A$는 양정치이다.
2. 모든 고유값이 양수이다.
3. 행 교환 없이 수행한 가우스 소거의 피보트(pivot)가 모두 양수이다.
...
**참고:** 실베스터 판정법은 양반정치 판정에는 사용할 수 없다. 양반정치의 필요충분조건은 **모든 주행렬식**(같은 인덱스 집합의 행과 열로 얻은 부분행렬의 행렬식; 선도 주행렬식만으로는 부족)이 0 이상인 것이다.
...
[← 이전: 대칭행렬·스펙트럼 정리·이차형식](spectral-theorem.html) · [다음: SVD →](svd.html)
