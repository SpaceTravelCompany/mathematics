...
(c) $W_3$는 부분공간이다. $x^2 + y^2 = 0$은 실수에서 $x = 0$이고 $y = 0$을 의미하므로 $W_3 = \{(0,0,z) \mid z \in \mathbb{R}\}$, 즉 $z$축 전체이다. $0 \in W_3$이고 덧셈과 스칼라배에 닫혀 있으므로 부분공간이다.
...
- **[직교성·직교투영·그람-슈미트](orthogonality.html)** : 함수공간에서의 직교 기저(예: 푸리에 급수의 삼각함수 기저)는 중요한 응용이다.
- **[푸리에 급수·푸리에 변환](fourier.html)** : 함수를 주파수 성분의 선형결합으로 분해하는 것은 벡터공간에서의 기저 전개로 이해할 수 있다.
- **[행렬곱과 선형변환](matrix-multiplication.html)** : 유한차원 벡터공간 사이의 선형변환은 항상 행렬로 표현된다.
- **[rank·열공간·널공간](rank-nullspace.html)** : rank-nullity 정리는 유한차원 벡터공간 사이의 모든 선형변환에 대해 성립한다.
- **[고유값·고유벡터](eigenvalues.html)** : 무한차원 함수공간에서의 고유값 문제는 미분방정식과 양자역학의 핵심이다.
