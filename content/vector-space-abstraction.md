...
> 학습 순서 17 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 벡터는 더하고 수를 곱할 수 있으며, 기저는 중복 없는 기본 재료다.

다항식 $p(x)=1+2x$, $q(x)=3-x$를 더하면 $p+q=4+x$다. $2p=2+4x$도 가능하다. 상수항과 $x$의 계수만 기록하면 각각 $(1,2)$, $(3,-1)$이라서 계산이 평면벡터의 계산과 똑같다.

차수 1 이하 다항식은 $a\cdot1+b\cdot x$로 모두 쓸 수 있으므로 기저는 $\{1,x\}$, 차원은 2다. 벡터가 반드시 화살표 모양이어야 하는 것이 아니라, 동일한 덧셈과 배수 규칙을 따르는 대상을 함께 부르는 것이다. $1$도 상수함수로서 여기의 벡터다.

부분공간인지 보려면 0을 포함하는지, 더해도 남는지, 어떤 실수배를 해도 남는지를 살핀다. ‘상수항이 1인 다항식’은 두 개를 더하면 상수항이 2가 되므로 부분공간이 아니다. 선형 미분방정식에서도 **동차** 방정식의 해들이 이런 구조를 가진다. 우변이 0이 아닌 방정식은 보통 그대로 벡터공간을 이루지 않는다.

**증명으로 이어 읽기:** 공리는 덧셈·배수 계산을 안전하게 옮겨 쓰기 위한 약속이다. 유한차원의 정리를 무한차원 함수공간에 옮길 때는 수렴 같은 추가 조건이 필요하다. 푸리에 급수의 ‘무한히 많은 재료’는 유한 선형결합과 그 극한을 구분해서 읽는다.
...
벡터공간(vector space)은 "덧셈과 스칼라배라는 두 연산이 정의된 집합"으로, 단 여덟 개의 공리(axiom)를 만족해야 한다. 이 추상화의 힘은, 벡터공간 공리만을 사용한 결과는 함수공간에서도 사용할 수 있다는 데 있다. 유한차원성이나 수렴 조건을 사용한 정리는 해당 가정을 따로 확인해야 한다. 예를 들어 $\mathbb{R}^n$에서의 기저와 차원 개념은 그대로 함수공간에 적용할 수 있다. 푸리에 해석은 바로 이 관점의 결정체다 — 함수 $\sin(nx)$와 $\cos(nx)$는 함수공간의 기저이며, 푸리에 급수는 함수를 이 기저들의 선형결합으로 표현하는 것이다.

양자역학에서 파동함수는 무한차원 벡터공간(힐베르트 공간)의 벡터이며, 신호 처리에서 신호는 함수공간의 원소다. 동차 선형 미분방정식의 해의 집합이 벡터공간을 이룬다는 사실(중첩 원리, superposition principle)은 선형 미분방정식 이론의 출발점이다.
...
[← 이전: 역행렬과 기저 변환](inverse-change-of-basis.html) · [다음: 내적·노름·코사인 유사도 →](inner-product-norm.html)
