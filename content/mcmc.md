...
> 학습 순서 67 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 몬테카를로는 표본평균으로 계산하고, 마르코프 체인은 상태를 확률적으로 이어 간다.

목표가 A를 80%, B를 20% 비율로 방문하는 것이라고 하자. 매번 다른 상태를 후보로 제안한다. 메트로폴리스 규칙에서는 현재가 A일 때 B를 $\min(1,0.2/0.8)=1/4$ 확률로 수락하고 나머지는 A에 머문다. B에서 A로 가는 후보는 항상 수락한다.

목표분포에서 A→B로 이동하는 비중은 $0.8\times0.25=0.2$, B→A도 $0.2\times1=0.2$다. 양방향 흐름이 맞아떨어지는 **상세균형** 덕분에 목표 비율이 유지된다. 일반 MH에서는 제안 자체가 비대칭일 수 있으므로 역방향과 정방향의 제안확률 비율도 넣는다.

목표밀도 $p(x)$의 공통 정규화 상수를 몰라도 $p(x')/p(x)$에서 상쇄되므로 샘플링 규칙을 만들 수 있다. 그러나 연속 표본끼리는 보통 상관되어 있어 1,000개를 모았다고 독립 표본 1,000개와 같은 정보량은 아니다.

**본문으로 이어 읽기:** burn-in은 시작점의 영향을 줄이려고 앞부분을 버리는 관행이지 수렴을 보증하는 마법의 횟수가 아니다. 상세균형은 정상성을 주지만 탐색 가능성과 수렴에는 별도 조건이 필요하다. 한 모드에 오래 갇히면 긴 실행도 목표 전체를 잘 대표하지 못할 수 있다.
...
[← 이전: 확률 행렬·마르코프 체인](markov-chains.html) · [다음: 확률미분방정식 →](sde.html)
