...
> 학습 순서 19 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 내적 0은 수직, 단위벡터는 길이 1인 벡터다.

$v=(3,4)$를 가로 성분 $(3,0)$과 세로 성분 $(0,4)$으로 나눌 수 있다. 서로 수직이므로 길이 제곱은 $3^2+4^2$처럼 더해진다. 한 성분을 바꿔도 다른 성분을 함께 고칠 필요가 없는 좌표계가 계산을 쉽게 한다.

임의의 단위벡터 $u$ 방향으로 남기는 성분은 $(v\cdot u)u$다. 먼저 내적으로 그 방향의 양을 숫자 하나로 구하고, 그 숫자만큼 $u$를 늘려 벡터를 되찾는다. 나머지 $r=v-(v\cdot u)u$는 $u$와 내적하면 0이므로 수직이다.

그람-슈미트는 새 벡터에서 기존 방향의 그림자를 빼는 과정이다. $v_1=(1,0)$, $v_2=(1,1)$이면 $v_2$에서 $(1,0)$을 빼 $(0,1)$을 얻는다. 각 결과를 자기 길이로 나누면 직교하면서 길이도 1인 **정규직교** 기저가 된다. 뺀 결과가 0이면 그 벡터는 중복된 재료였으므로 0으로 나누지 않고 제외한다.

**증명으로 이어 읽기:** 가장 가까운 점에서 오차가 수직이어야 하는 이유는, 오차에 평행 성분이 남으면 그 방향으로 조금 움직여 거리를 더 줄일 수 있기 때문이다. 이 사실이 다음 최소제곱법의 핵심이다.
...
[← 이전: 내적·노름·코사인 유사도](inner-product-norm.html) · [다음: 최소제곱법 →](least-squares.html)
