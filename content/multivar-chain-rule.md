...
> 학습 순서 34 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 한 경로를 따라 전달되는 변화율은 곱하고, 여러 입력의 기여는 더한다.

$z=x^2+y$, $x=t$, $y=3t$라 하자. $t$가 변하면 $x$를 거쳐 $z$로 가는 길과 $y$를 거쳐 $z$로 가는 길이 모두 있다. 각 길의 변화율은 $(2x)\cdot1$과 $1\cdot3$이므로

$$\frac{dz}{dt}=\frac{\partial z}{\partial x}\frac{dx}{dt}+\frac{\partial z}{\partial y}\frac{dy}{dt}=2t+3.$$

실제로 $z=t^2+3t$로 먼저 합성한 뒤 미분해도 같다. 길마다 곱한 뒤 더한다는 규칙은 중간 변수가 많아져도 동일하다.

역전파는 최종 출력에서 출발해 각 중간값의 기여를 거꾸로 모으는 계산 순서다. 하나의 중간값이 두 곳에 쓰이면 두 경로의 기여를 **더해야** 한다. 신경망이라는 이름과 무관하게 같은 연쇄법칙이다.

**증명으로 이어 읽기:** 여러 출력을 한꺼번에 다루면 변화율들을 행렬에 적는다. 이것이 야코비안이며, 합성의 야코비안 곱은 ‘각 경로에서 곱하고 모든 경로를 더한다’는 행렬곱의 계산과 같다. 바로 뒤의 야코비안 장에서 행과 열의 위치를 구체화한다.
...
[← 이전: 방향 도함수](directional-derivative.html) · [다음: 2계 도함수·헤시안·곡률 →](second-derivatives.html)
