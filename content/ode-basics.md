...
> 학습 순서 48 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 도함수는 현재의 변화율, 적분은 그 변화의 누적이다.

$x'(t)=2$는 ‘매 시간 2씩 늘어나는 양’을 찾으라는 문제다. 답은 $x(t)=2t+C$다. $x(0)=3$이라는 초기조건이 주어지면 $C=3$으로 하나를 고를 수 있다. 미지수가 숫자 하나가 아니라 **함수 전체**라는 점이 보통 방정식과 다르다.

$x'=x$는 현재 양이 클수록 더 빨리 늘어난다. $x(t)=Ce^t$를 미분하면 다시 $Ce^t$가 되어 식을 만족한다. $x(0)=2$이면 $x(t)=2e^t$다. 계산한 함수를 식에 넣어 읽어 보면 왜 해인지 알 수 있다.

정확한 식을 못 구해도 짧은 시간 $h$ 뒤를 $x(t+h)\approx x(t)+h f(x(t),t)$로 근사할 수 있다. 이것이 오일러 방법이다. 현재 상태에서의 속도를 다음 짧은 구간 동안 유지한다고 보는 것이므로 보폭이 크면 오차가 커질 수 있다.

**증명으로 이어 읽기:** 존재는 해가 있다는 뜻, 유일성은 같은 초기조건에서 하나라는 뜻이다. 충분조건인 국소 Lipschitz 조건은 가까운 두 상태의 변화율 차이를 상태 차이의 일정 배수로 제한한다. 우변이 연속이라는 사실만으로 유일성까지 보장되지는 않는다. 변수분리 중 $x$로 나누면 $x=0$ 같은 해가 빠질 수 있다.
...
[← 이전: 그린·스토크스·가우스 정리](stokes-theorems.html) · [다음: 미분방정식과 동역학 →](dynamical-systems.html)
