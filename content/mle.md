...
> 학습 순서 60 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 확률모형의 모수는 아직 모르는 설정값이고, 독립 관측들의 확률은 곱해진다.

동전을 세 번 던져 앞·앞·뒤를 관측했다고 하자. 앞면 확률 후보를 $p$라고 두면 이 순서의 가능도는 $L(p)=p^2(1-p)$다. $p=1/2$에서는 $1/8$, $p=2/3$에서는 $4/27$이므로 후자가 이 관측을 더 잘 설명한다.

$L'(p)=2p-3p^2=p(2-3p)$를 풀면 내부 후보는 $2/3$이다. 양 끝 0과 1에서는 가능도가 0이고, 내부에서 증가하다 감소하므로 최댓값은 $p=2/3$에서 나온다. 이것이 최대가능도추정이다.

$\log L=2\log p+\log(1-p)$를 최대화해도 같은 답이다. 로그는 증가함수라 순위를 바꾸지 않고 곱을 합으로 바꿔 계산을 쉽게 한다. 연속형 데이터에서는 한 점의 확률이 아니라 **확률밀도**를 모수의 함수로 본다.

**본문으로 이어 읽기:** 가능도는 관측값을 고정하고 모수를 움직인다. 그래서 모수에 대해 적분한 값이 1일 필요가 없으며, 그 자체를 모수의 확률분포로 읽으면 안 된다. 도함수 0은 내부 후보 조건이므로 경계, 최댓값의 존재, 여러 후보도 함께 고려한다.
...
[← 이전: 조건부확률의 함정](conditional-traps.html) · [다음: 베이지안 추론 →](bayesian-inference.html)
