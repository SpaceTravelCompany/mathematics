...
> 학습 순서 75 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 조건부확률은 관측한 집단을 골라 보는 것이고, 회귀는 관계를 추정한다. 인과 질문은 선택을 실제로 바꾸면 무엇이 달라질지 묻는다.

우산을 든 날에 젖은 신발이 많아도 우산 때문에 신발이 젖었다고 결론 내릴 수 없다. 비가 우산 사용과 신발 젖음을 함께 늘렸을 수 있다. 이때 비는 두 변수에 영향을 주는 교란변수다.

$P(Y|X=x)$는 원래 $X=x$였던 집단을 본다. $P(Y|\operatorname{do}(X=x))$는 사람들의 선택 과정 대신 외부에서 $X$를 $x$로 정한 상황을 본다. ‘우산 든 사람을 모으기’와 ‘우산을 무작위로 나눠 주기’의 차이다.

집단 $Z$가 적절한 조정 변수라면 집단별 개입 효과를 같은 집단 비중으로 평균하여 $P(Y|\operatorname{do}(X=x))=\sum_zP(Y|X=x,Z=z)P(Z=z)$를 사용할 수 있다. 예를 들어 두 집단의 성공률이 0.8, 0.4이고 목표집단 비중이 반반이면 보정한 성공률은 0.6이다. 관측된 처리집단의 구성비를 그대로 쓰는 것과 다르다.

**본문으로 이어 읽기:** 아무 변수나 많이 보정한다고 인과효과가 드러나는 것은 아니다. 두 원인의 공통 결과인 충돌변수에 조건을 걸면 오히려 관계가 생길 수 있다. 백도어 조정은 특정한 그래프 조건, 도구변수는 관련성·외생성·배제 제약 같은 가정이 필요하다. 무작위 배정도 유한 표본에서 두 집단의 특성이 정확히 일치한다는 보장은 아니며, 배정과 잠재결과의 독립성을 설계로 확보한다는 뜻이다.
...
[← 이전: 임베딩 공간의 기하학](embedding-geometry.html)
