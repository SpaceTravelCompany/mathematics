...
> 학습 순서 31 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 도함수는 입력 하나를 움직일 때의 변화율이다.

$f(x,y)=x^2+3y$는 숫자 두 개를 받아 하나를 내놓는다. $x$만 움직일 때 $y$는 고정한 숫자이므로 $\partial f/\partial x=2x$다. $y$만 움직이면 $x^2$가 상수이므로 $\partial f/\partial y=3$이다. 둥근 모양의 $\partial$는 여러 입력 중 하나만 바꾼다는 표시다.

$(1,2)$에서 두 변화율을 모으면 $\nabla f=(2,3)$이다. $\nabla$는 나블라라고 읽고, 여기서는 편도함수들을 모은 그래디언트 연산자다. 작은 이동 $(\Delta x,\Delta y)$에 대한 출력 변화는 대략 $2\Delta x+3\Delta y$다. $(0.01,-0.02)$만큼 이동하면 약 $-0.04$이고, 실제 변화는 $-0.0399$다. 작은 차이는 $x^2$에서 생긴 이차항 $0.01^2$다.

**증명으로 이어 읽기:** 다변수 미분가능성은 모든 작은 이동에 이 선형 근사가 잘 맞는다는 뜻이다. 축 방향 편도함수가 각각 존재하기만 한다고 다른 방향까지 좋은 근사가 보장되지는 않는다. 편도함수들이 주변에서 연속이면 미분가능성을 보장할 수 있다.
...
[← 이전: 급수와 수렴](series-convergence.html) · [다음: 등고선과 그래디언트 →](gradient-geometry.html)
