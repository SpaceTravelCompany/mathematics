...
> 학습 순서 16 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 역함수는 출력을 입력으로 되돌리고, 행렬식 0은 정사각행렬이 정보를 잃었다는 뜻이다.

$A=\left(\begin{smallmatrix}2&0\\0&3\end{smallmatrix}\right)$는 두 성분을 각각 2배, 3배 한다. 이를 되돌리는 $A^{-1}=\left(\begin{smallmatrix}1/2&0\\0&1/3\end{smallmatrix}\right)$를 뒤에 적용하면 원래 벡터로 돌아온다. $A^{-1}A=I$의 $I$는 아무것도 바꾸지 않는 항등행렬이다. 위의 $-1$은 행렬 각 성분의 역수를 취하라는 명령이 아니다.

$A(x,y)=(x,0)$처럼 세로 정보를 지우면 출력만 보고 원래 $y$를 알 수 없으므로 역행렬이 없다. 늘리거나 기울이는 변형 자체가 문제인 것이 아니라 입력들이 하나로 합쳐져 정보가 없어지는 것이 문제다.

기저 $b_1=(2,0)$, $b_2=(0,1)$로 벡터 $(6,4)$를 읽으면 $3b_1+4b_2$이므로 새 좌표는 $(3,4)$다. 기저를 열로 모은 $P$는 새 좌표를 원래 좌표로 보내고, $P^{-1}$은 원래 좌표를 새 좌표로 보낸다.

**증명으로 이어 읽기:** $P^{-1}AP$는 새 좌표에서 출발해 $P$로 원래 좌표로 바꾸고, $A$를 적용한 뒤, $P^{-1}$로 새 좌표로 돌아오는 세 단계다. 같은 변환을 다른 좌표로 적은 것이므로 공간에서 하는 일은 같다.

---

## 직관적 설명

어떤 변환을 **되돌리는** 것이 가능할까? 같은 유한 차원의 공간 사이의 선형변환 $T$가 차원을 줄이지 않으면, $T$를 되돌리는 역변환 $T^{-1}$이 존재한다. 길이·각도·모양은 달라져도 되며, 핵심은 입력 정보가 사라지지 않는다는 것이다. 행렬의 언어로는 $A^{-1}$이 **역행렬(inverse matrix)**이다. $A^{-1} A = I = A A^{-1}$이 성립하며, 이는 "변환 후 되돌리면 원래대로"라는 뜻이다.
...
이 개념의 실용적 예로는 서로 다른 직교축으로 벡터 성분을 읽는 작업, 카메라 좌표계의 회전 부분, 결정 구조의 단위세포 기저 변경 등이 있다. 기저 변환은 **관점의 변화**를 수학적으로 정밀하게 표현하는 도구다.
...
[← 이전: 행렬식의 기하학](determinant.html) · [다음: 벡터공간 추상화 →](vector-space-abstraction.html)
