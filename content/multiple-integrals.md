...
> 학습 순서 43 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 적분은 ‘조각의 크기 × 그 조각의 값’을 모두 더한다. 야코비안은 작은 좌표변화의 배율을 알려준다.

가로 2, 세로 3인 판의 면적밀도가 일정하게 4라면 질량은 $2\times3\times4=24$다. 밀도가 $f(x,y)$로 달라지면 작은 직사각형마다 $f(x,y)\Delta x\Delta y$를 더한다. 이것이 $\iint_D f(x,y)\,dA$다.

삼각형 $D=\{0\le x\le1,\ 0\le y\le x\}$의 넓이는 $\int_0^1\int_0^x1\,dy\,dx$다. 안쪽 적분은 $x$를 고정하고 세로 조각의 길이 $x$를 구한다. 바깥 적분은 그 길이를 가로로 더해 $\int_0^1x\,dx=1/2$를 얻는다. 적분 범위는 그림에서 각 조각이 시작하고 끝나는 위치다.

극좌표 $x=r\cos\theta,y=r\sin\theta$에서는 바깥쪽으로 갈수록 같은 각도 폭이 더 긴 호를 만든다. 작은 면적은 $dr\times r\,d\theta$이므로 $dA=r\,dr\,d\theta$다. 이 $r$이 야코비안의 절댓값이다.

**증명으로 이어 읽기:** 적분 순서를 바꾸는 정리는 임의의 식을 무조건 바꾸라는 말이 아니다. 음이 아닌 함수에는 토넬리 정리, 절대적분 가능한 함수에는 푸비니 정리가 적용된다. 음수인 함수값이 있으면 적분은 보통 기하학적 부피가 아니라 부호 있는 누적량이다.
...
[← 이전: 텐서 연산](tensor-operations.html) · [다음: 푸리에 급수·푸리에 변환 →](fourier.html)
