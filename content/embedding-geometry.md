...
> 학습 순서 74 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 벡터로 숫자들을 모으고, 거리와 각도로 벡터 사이 관계를 잴 수 있다.

문서 하나를 ‘요리 관련 정도, 운동 관련 정도’라는 두 숫자로 나타낸다고 하자. $a=(1,0)$와 $b=(2,0)$는 크기는 다르지만 같은 방향이므로 코사인 유사도는 1이다. $c=(0,1)$은 수직이므로 $a$와 코사인 유사도가 0이다. 임베딩은 이런 숫자 표현을 많은 차원으로 확장한 것이다.

표현을 잘 학습했을 때 가까운 벡터들이 의미도 비슷할 수 있다. 그러나 벡터로 바꾸었다는 사실만으로 의미 관계가 자동 보장되지는 않는다. 어떤 데이터와 목표로 표현을 만들었는지가 중요하다.

차원의 저주를 작은 예로 보자. 한 축을 10칸으로 나누면 1차원은 10칸, 2차원은 100칸, 10차원은 $10^{10}$칸이다. 각 축을 같은 세밀함으로 채우려면 필요한 표본이 급격히 늘어난다. 이것이 고차원에서 ‘주변에 충분한 데이터가 있다’고 말하기 어려워지는 한 이유다.

**본문으로 이어 읽기:** 거리 집중이나 무작위 벡터의 거의 직교성은 독립성·등방성 같은 분포 가정 아래의 현상이다. 모든 고차원 데이터가 자동으로 그런 모양인 것은 아니다. PCA는 평균을 뺀 데이터의 분산이 큰 방향을 고르는 선형 방법이며, 큰 분산이 항상 작업에 중요한 의미라는 뜻도 아니다.
...
[← 이전: 정보기하·자연 그래디언트](information-geometry.html) · [다음: 인과추론 →](causal-inference.html)
