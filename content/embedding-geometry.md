...
- **[내적·노름·코사인 유사도](topics/inner-product-norm.html)** : 임베딩 공간의 거리와 각도를 측정하는 기본 도구로, 고차원 기하학의 모든 계산은 내적과 노름에서 출발한다.
- **[정보기하·자연 그래디언트](topics/information-geometry.html)** : 확률분포 공간은 정보기하학적 관점에서 리만 매니폴드로 이해되며, 고차원 임베딩 공간의 기하학과 밀접하게 연결된다.
- **[가우시안 과정](topics/gaussian-process.html)** : 고차원 함수 공간 위의 확률분포로, 커널 함수에 의해 결정되는 기하학적 구조를 가진다.
- **[주성분 분석(PCA)](topics/svd.html)** : 고차원 데이터의 분산이 가장 큰 방향을 찾는 선형 차원 축소 방법으로, 임베딩 공간의 기하학적 구조를 단순화한다.
