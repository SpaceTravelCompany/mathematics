...
> 학습 순서 62 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 로그는 곱을 합으로 바꾸고, 기댓값은 확률로 가중한 평균이다.

확률 $p$인 결과의 놀람을 $-\log_2p$로 재면, 확률 $1/2$는 1비트, $1/4$는 2비트다. 더 드문 결과일수록 놀람이 크다. 공정한 동전은 두 결과가 모두 1비트이므로 평균 놀람인 엔트로피도 1비트다. 결과가 확정되어 있으면 $-\log_2 1=0$이다.

실제 분포 $p=(1/2,1/2)$를 예상 분포 $q=(3/4,1/4)$로 설명하면 평균 비용은 $-\frac12\log_2(3/4)-\frac12\log_2(1/4)\approx1.208$비트다. 이 **교차엔트로피**에서 원래 엔트로피 1을 뺀 약 0.208이 KL 발산이다.

KL은 $p$에서 자주 나오는 결과를 $q$가 얼마나 잘 설명하는지 잰다. 순서를 바꾸면 평균을 내는 기준도 바뀌므로 보통 값이 다르다. $p$가 양수인 결과에 $q=0$을 주면 KL은 무한대가 될 수 있다. 대칭성과 삼각부등식을 요구하는 수학적 거리와는 다르다.

**본문으로 이어 읽기:** 상호정보량은 두 변수의 실제 결합분포와 ‘독립이라고 가정한 곱 분포’ 사이의 KL이다. 0이면 독립이다. 이산 엔트로피와 달리 연속형의 미분엔트로피는 음수일 수 있으며 좌표 단위를 바꾸면 값이 변한다. KL의 비음성은 이 차이와 별개다.
...
[← 이전: 베이지안 추론](bayesian-inference.html) · [다음: 몬테카를로 →](monte-carlo.html)
