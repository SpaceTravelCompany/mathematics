...
> 학습 순서 61 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 베이즈 정리는 증거를 본 뒤 각 가설의 비중을 갱신한다. 여기서는 가설이 모수값이다.

동전 A는 앞면 확률 $1/2$, B는 $3/4$이며 둘을 똑같이 가능하다고 생각한다고 하자. 앞면을 한 번 본 뒤의 미정규화 비중은 A가 $(1/2)(1/2)=1/4$, B가 $(1/2)(3/4)=3/8$이다. 합 $5/8$로 각각 나누면 사후확률은 A가 $2/5$, B가 $3/5$다.

‘사후 ∝ 가능도 × 사전’의 $\propto$는 공통 상수로 나누기 전의 비율이 같다는 뜻이다. 가능도만 비교하는 MLE와 달리, 사전에 얼마나 가능하게 보았는지도 함께 반영한다. 사전은 모수에 대한 불확실성을 적은 분포이지 틀림없는 사실이 아니다.

다음 앞면 확률을 예측할 때는 한 동전만 선택하지 않고 $(2/5)(1/2)+(3/5)(3/4)=13/20$으로 평균한다. 이것이 사후예측의 작은 예다.

**본문으로 이어 읽기:** 베타분포의 밀도는 $p^{\alpha-1}(1-p)^{\beta-1}$에 비례한다. 앞면 $h$번, 뒷면 $t$번의 가능도를 곱하면 지수만 더해져 $\operatorname{Beta}(\alpha+h,\beta+t)$가 된다. 사전과 사후가 같은 분포 가족에 남는 이런 관계를 공액이라고 부른다.
...
[← 이전: 최대가능도추정 MLE](mle.html) · [다음: 엔트로피·KL발산·상호정보량 →](entropy-kl.html)
