...
> 학습 순서 33 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 그래디언트는 축별 변화율, 단위벡터는 순수한 방향을 나타낸다.

한 점의 그래디언트가 $(2,3)$이고 $u=(3/5,4/5)$ 방향으로 걷는다고 하자. $u$의 길이는 1이다. 단위거리당 변화율은

$$D_u f=\nabla f\cdot u=2\cdot\frac35+3\cdot\frac45=\frac{18}{5}.$$

즉 그 방향으로 아주 짧게 $h$만큼 움직이면 높이는 약 $(18/5)h$만큼 변한다. 그래디언트 방향의 단위벡터 $(2,3)/\sqrt{13}$에서는 변화율이 $\sqrt{13}$으로 최대가 된다.

길이 2인 벡터를 그대로 넣으면 같은 시간에 두 배 멀리 움직이는 경로의 변화율을 계산하게 된다. ‘거리당 경사’를 비교할 때 길이를 1로 맞추는 이유다.

**증명으로 이어 읽기:** $g(t)=f(x+tu)$라는 한 변수 함수를 만들고 $t=0$에서 미분하면 방향 도함수가 된다. 다변수 연쇄법칙이 내적 공식을 준다. 모든 방향 도함수가 존재한다는 조건만으로 함수 전체의 미분가능성이 보장되지는 않는다.
...
[← 이전: 등고선과 그래디언트](gradient-geometry.html) · [다음: 다변수 연쇄법칙 →](multivar-chain-rule.html)
