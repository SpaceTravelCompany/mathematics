...
> 학습 순서 13 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 행렬의 각 행은 방정식 하나의 계수들을 적은 것이다.

$x+y=3$, $2x+y=4$를 풀자. 둘째 식에서 첫째 식을 빼면 $x=1$이다. 첫째 식에 넣으면 $1+y=3$이므로 $y=2$다. 소거는 답을 짐작하는 대신 식끼리 계산해서 변수 하나를 없애는 방법이다.

숫자만 적은 확대행렬은 $\left(\begin{smallmatrix}1&1&|&3\\2&1&|&4\end{smallmatrix}\right)$이다. 세로줄 오른쪽은 등호 오른쪽 값이다. 행을 바꾸기, 행에 0이 아닌 수를 곱하기, 다른 행의 배수를 더하기는 모두 되돌릴 수 있으므로 해를 보존한다.

소거 뒤 $0x+0y=1$이 나오면 해가 없다. $0x+0y=0$이면 새 제약이 없는 줄이다. 그때 변수를 전부 결정할 독립적인 식이 부족하면 남는 변수를 자유롭게 정할 수 있어 해가 무한히 많다. RREF는 각 기준 자리의 값을 1로 만들고 그 열의 다른 값을 0으로 만든 최종 정리 형태다. 그 기준 자리를 피보트라 한다.

**증명으로 이어 읽기:** 기본 행 연산의 가역성은 해 보존의 이유이고, 피보트와 자유변수의 개수는 해의 모양을 설명한다. 다음 장의 rank는 피보트의 개수, nullity는 자유변수의 개수와 연결된다.
...
[← 이전: 선형결합·span·일차독립](span-independence.html) · [다음: rank·열공간·널공간 →](rank-nullspace.html)
