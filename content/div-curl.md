...
> 학습 순서 45 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 벡터장은 각 위치에 화살표 하나를 놓는 함수이고, 편도함수는 화살표 성분의 위치별 변화를 잰다.

$F(x,y)=(x,y)$를 그리면 화살표가 원점에서 바깥으로 향한다. 발산은 $\partial F_x/\partial x+\partial F_y/\partial y=1+1=2$다. 작은 상자를 놓았을 때 들어오는 것보다 나가는 흐름이 많다는 뜻이다. 발산은 순 유출량 자체가 아니라 **단위 면적·부피당 순 유출량의 극한**이다.

$G(x,y)=(-y,x)$는 원 주위를 도는 흐름이다. 발산은 0이지만 2차원 회전 성분은 $\partial G_y/\partial x-\partial G_x/\partial y=1-(-1)=2$다. 따라서 ‘순수하게 빠져나가는 양이 없다’와 ‘돌지 않는다’는 다른 말이다. 3차원 회전은 회전축 방향까지 담은 벡터다.

**증명으로 이어 읽기:** $\nabla$는 숫자 벡터가 아니라 미분 연산자들의 표기다. 내적 모양으로 조합하면 발산, 외적 모양으로 조합하면 회전이다. 혼합 편도함수를 교환할 수 있는 매끄러움 아래 $\nabla\times\nabla f=0$, $\nabla\cdot(\nabla\times F)=0$이 성립한다. 회전 0에서 전역적인 퍼텐셜 존재로 넘어갈 때는 영역의 구멍 같은 조건도 필요하다.
...
[← 이전: 푸리에 급수·푸리에 변환](fourier.html) · [다음: 선적분·면적분 →](line-surface-integrals.html)
