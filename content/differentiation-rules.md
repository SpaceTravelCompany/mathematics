...
> 학습 순서 27 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 미분은 아주 작은 입력 변화가 출력으로 전달되는 비율이다.

$y=(3x+1)^2$를 계산할 때 먼저 $u=3x+1$, 다음 $y=u^2$라는 두 단계를 거친다. $x$ 변화는 $u$에서 3배가 되고, $u$ 변화는 $y$에서 현재 위치의 $2u$배가 된다. 전체 비율은 $2u\times3=6(3x+1)$이다. 이것이 연쇄법칙이다.

곱 $f(x)g(x)$에서는 한쪽만 변하는 것이 아니다. 작은 변화량을 $\Delta f,\Delta g$라 하면 곱의 변화는 $g\Delta f+f\Delta g+\Delta f\Delta g$다. 마지막 항은 작은 변화 두 개의 곱이어서 미분의 극한에서 사라지고 $(fg)'=f'g+fg'$가 남는다. 그래서 $(fg)'=f'g'$로 쓰면 틀린다.

예를 들어 $x^2\cdot x^3=x^5$를 곱 법칙으로 미분하면 $2x\cdot x^3+x^2\cdot3x^2=5x^4$다. 몫 법칙은 분모가 0이 아닌 곳에서 적용한다.

**증명으로 이어 읽기:** 먼저 함수가 합·곱·몫·합성 중 어떤 구조인지 나누고 해당 법칙을 쓴다. $dy/dx$의 분수 모양은 변화율을 기억하는 데 도움이 되지만 언제나 보통 분수처럼 약분할 수 있는 것은 아니다. 연쇄법칙이 그 연결을 정당화한다.
...
- **[지수·로그 함수의 미분](exp-log-calculus.html)** : $e^x$의 미분이 자기 자신임은 연쇄법칙과 함께 지수함수 미분의 핵심이 된다. 역함수 미분법으로 $\ln x$의 미분을 유도한다.
- **[다변수 연쇄법칙](multivar-chain-rule.html)** : 연쇄법칙을 다변수 함수로 확장하면 편미분의 행렬곱(야코비안 곱)으로 표현되며, 이는 합성함수의 미분을 행렬 연산으로 일반화한 것이다.

---

[← 이전: 극한·연속·도함수](limits-derivatives.html) · [다음: 지수·로그 함수의 미분 →](exp-log-calculus.html)
