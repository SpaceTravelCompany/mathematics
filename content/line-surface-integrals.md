...
> 학습 순서 46 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 내적은 이동 방향으로 작용하는 성분을 뽑고, 적분은 작은 조각의 기여를 더한다.

일정한 힘 $F=(2,0)$을 받으며 $(0,0)$에서 $(3,0)$으로 가면 일은 $F\cdot(3,0)=6$이다. 같은 힘 아래 $(0,0)$에서 $(0,3)$으로 가면 내적이 0이므로 일도 0이다. 경로가 휘어 있으면 작은 이동 $dr$마다 $F\cdot dr$을 더해 선적분을 구한다.

경로 $r(t)=(t,t)$, $0\le t\le1$에서는 $dr=(1,1)dt$이므로 같은 힘의 일은 $\int_0^1 2\,dt=2$다. 반면 밀도 1인 이 선의 길이는 $\int_0^1\|r'(t)\|dt=\sqrt2$다. $F\cdot dr$과 $f\,ds$는 서로 다른 양을 적분한다.

면을 지나는 흐름은 면의 단위 법선 $n$과 내적한 $F\cdot n$에 면적을 곱해 더한다. 법선은 표면에 수직인 방향이다. 방향을 뒤집으면 플럭스의 부호도 뒤집힌다. 스칼라 면적분 $\iint_S f\,dS$는 방향 없이 표면의 밀도 등을 누적한다.

**본문으로 이어 읽기:** 매개화는 곡선·곡면 위 위치를 새 입력 $t$ 또는 $(u,v)$로 적는 일이다. 길이와 면적의 배율이 각각 $\|r'(t)\|$, $\|r_u\times r_v\|$로 들어간다. 범위를 정하고, 이동 또는 법선 방향을 정하고, 배율을 곱하는 순서로 읽자.
...
[← 이전: 발산·회전](div-curl.html) · [다음: 그린·스토크스·가우스 정리 →](stokes-theorems.html)
