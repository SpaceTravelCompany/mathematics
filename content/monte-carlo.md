...
> 학습 순서 63 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 적분으로 쓴 기댓값도 표본을 많이 얻으면 평균으로 근사할 수 있다.

$I=\int_0^1 x^2dx=1/3$을 모른다고 하자. $[0,1]$에서 균등하게 뽑은 $U$의 밀도는 1이므로 $I=\mathbb E[U^2]$다. 따라서 독립 표본 $U_1,\ldots,U_N$을 얻어 제곱의 평균을 내면 적분을 근사한다.

설명을 위해 표본이 $0.2,0.5,0.9$였다고 하면 계산값은 $(0.04+0.25+0.81)/3\approx0.367$이다. 표본을 적게 뽑았으므로 참값과 다를 수 있다. 대수의 법칙이 표본 수를 늘릴 때의 수렴을 설명하고, 중심극한정리가 오차 규모를 설명한다.

독립 표본의 함수값 분산이 유한하면 평균의 표준편차는 원래 표준편차를 $\sqrt N$으로 나눈 값이다. 오차를 절반 수준으로 줄이려면 보통 표본을 네 배쯤 사용한다. 고차원에서도 지수 $-1/2$는 같지만 분산이나 한 표본의 계산 비용까지 차원과 무관한 것은 아니다.

**본문으로 이어 읽기:** $[a,b]$ 균등표본을 쓰면 밀도가 $1/(b-a)$이므로 일반 적분은 $(b-a)$를 표본평균에 곱해야 한다. 다른 분포에서 뽑는 경우의 보정은 뒤의 중요도 샘플링에서 자세히 다룬다.
...
[← 이전: 엔트로피·KL발산·상호정보량](entropy-kl.html) · [다음: 가설검정·p-value·신뢰구간 →](hypothesis-testing.html)
