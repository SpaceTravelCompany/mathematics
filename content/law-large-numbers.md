...
> 학습 순서 57 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 기댓값은 분포가 정하는 평균, 표본평균은 실제로 뽑은 데이터의 평균이다.

공정한 동전을 $n$번 독립적으로 던지고 앞면이면 $X_i=1$, 뒷면이면 0이라고 적자. 표본평균 $\bar X_n=(X_1+\cdots+X_n)/n$은 앞면 비율이다. 대수의 법칙은 시행 수가 커지면 이 비율이 $1/2$에 가까워진다는 뜻이다.

이는 100번에서의 비율보다 101번에서의 비율이 반드시 더 가깝다는 뜻은 아니다. 도중에 멀어지는 구간도 있다. 또한 앞면이 많이 나왔으니 다음에 뒷면이 더 잘 나온다는 보상 법칙도 아니다. 독립 동전의 다음 앞면 확률은 여전히 $1/2$다.

동전 한 번의 분산은 $1/4$이고 평균의 분산은 $1/(4n)$이다. 체비쇼프 부등식은 $P(|\bar X_n-1/2|\ge\epsilon)\le1/(4n\epsilon^2)$를 준다. 허용오차 $\epsilon$을 고정하면 크게 틀릴 확률의 상한이 0으로 내려간다.

**증명으로 이어 읽기:** 약한 법칙은 ‘허용오차를 넘길 확률’이 0으로 간다는 말이고, 강한 법칙은 무한히 이어지는 표본 경로들 중 확률 1의 경로에서 평균이 수렴한다는 말이다. 기본적인 독립·동일분포 형태에서는 유한한 절댓값 기댓값이 필요하며, 위의 짧은 분산 증명은 유한분산을 추가로 사용한다.
...
### 정리 5: 체르노프 부등식 (Chernoff Bound, 서술)
...
**예제 7 (분산이 무한대인 경우의 WLLN):** 분산이 존재하지 않는 분포에서도 WLLN이 성립할 수 있을까? 일반적으로 분산이 무한대이면 체비셰프 부등식을 통한 증명은 작동하지 않지만, **약한 대수의 법칙은 분산이 유한하지 않아도 성립할 수 있다**($\mathbb{E}[|X|] < \infty$면 충분하다는 것이 크네친-콜모고로프 정리). 단, 증명은 더 복잡한 절단(truncation) 기법을 필요로 한다.
...
[← 이전: 결합·주변·조건부 분포](joint-marginal-conditional.html) · [다음: 중심극한정리 →](clt.html)
