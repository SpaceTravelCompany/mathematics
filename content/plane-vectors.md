...
> 학습 순서 9 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 좌표 두 개로 위치를 나타내고, 두 점의 차이로 이동을 나타낸다.

$(3,2)$라는 벡터는 ‘오른쪽 3, 위쪽 2만큼 이동’이라고 읽자. 이어서 $(-1,4)$만큼 이동하면 전체 이동은 $(3-1,2+4)=(2,6)$이다. 두 배의 이동은 $2(3,2)=(6,4)$다. 이때 곱한 하나의 수를 스칼라라고 한다.

벡터 $(3,4)$의 길이는 $\sqrt{3^2+4^2}=5$다. 길이 1로 만들려면 전체를 5로 나누어 $(3/5,4/5)$로 만든다. 이를 단위벡터라고 한다. 영벡터 $(0,0)$는 길이가 0이라 나눌 수 없으며 방향도 정하지 않는다.

내적은 같은 자리끼리 곱해 더하는 수다. $(2,1)\cdot(3,-2)=2\times3+1\times(-2)=4$다. $(1,0)\cdot(0,1)=0$은 가로 이동과 세로 이동이 서로 수직임을 나타낸다. 벡터끼리 계산했다고 결과가 항상 벡터인 것은 아니다. 내적의 결과는 숫자 하나다.

**증명으로 이어 읽기:** 투영은 한 방향으로 남는 성분, 즉 그림자다. $(3,4)$를 가로축에 투영하면 $(3,0)$이다. 일반 방향에서는 단위벡터 $u$에 대해 $(v\cdot u)u$로 같은 작업을 한다. 길이와 방향을 함께 보는 내적을 이후 행렬곱과 직교성에서도 사용한다.
...
- **[내적과 노름](inner-product-norm.html)** : 평면벡터의 내적과 노름을 고차원 벡터공간으로 일반화한다. 이 문서는 2차원에서의 입문을 다룬다.
...
[← 이전: 좌표기하와 이차곡선](coordinate-geometry.html) · [다음: 복소수 — 회전을 숫자로 쓰기 →](complex-numbers.html)
