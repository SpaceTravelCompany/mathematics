...
> 학습 순서 44 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 직교투영은 원하는 방향의 성분을 뽑고, 삼각함수는 반복되는 파형이다. 복소 지수는 회전을 짧게 쓴 표현이다.

$f(t)=3\cos t+2\sin(2t)$에는 한 주기에 한 번 진동하는 코사인 성분과 두 번 진동하는 사인 성분이 섞여 있다. 이 둘의 양 3과 2를 찾아내는 것이 푸리에 분석의 출발점이다. 주파수는 일정 시간에 몇 번 반복하는지, 진폭은 얼마나 크게 흔들리는지를 뜻한다.

벡터에서 내적으로 성분을 뽑았듯, 함수에서는 한 주기 동안 곱을 적분한다. $[-\pi,\pi]$에서 서로 다른 사인·코사인 성분의 곱은 양수와 음수가 상쇄되어 적분이 0이다. 따라서

$$\frac1\pi\int_{-\pi}^{\pi} f(t)\cos t\,dt=3.$$

자기 자신을 곱한 $\cos^2t$의 적분은 $\pi$이므로 $1/\pi$로 나누었다. 이것이 계수 공식의 이유다. 복소수 표현 $e^{i\omega t}$는 사인·코사인 두 성분을 함께 다룬다.

**본문으로 이어 읽기:** 급수는 주기 신호를 이산적인 주파수 목록으로, 변환은 적절한 비주기 신호를 연속적인 주파수로 다룬다. 복원은 함수의 조건과 수렴의 의미에 달려 있다. 예를 들어 점프가 있는 구간별 매끄러운 주기 함수에서는 그 지점의 좌우 극한 평균으로 수렴하므로 ‘모든 점에서 원래 값’이라고 말할 수 없다.

---

## 직관적 설명

**푸리에 급수(Fourier series)** 는 적절한 조건을 만족하는 주기 함수를 사인과 코사인의 무한합으로 표현한다. 제곱적분 가능한 함수의 평균제곱 수렴과, 구간별 매끄러운 함수의 점별 수렴은 서로 구분한다. 마치 프리즘(prism)이 백색광을 스펙트럼 색상으로 분해하듯, 푸리에 급수는 시간 영역의 신호를 주파수 성분으로 분해한다. 복잡한 파형(music, 음성, 전파)이 단순한 정현파(sinusoid)들의 합성이라는 사실은 신호 처리, 양자역학, 편미분방정식 해법의 기초가 된다.
...
[← 이전: 다중적분](multiple-integrals.html) · [다음: 발산·회전 →](div-curl.html)
