...
> 학습 순서 40 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 지수는 항상 양수이고, 연쇄법칙으로 합성함수를 미분한다.

시그모이드 $\sigma(x)=1/(1+e^{-x})$에 0을 넣으면 $1/2$다. 큰 양수에서는 $e^{-x}$가 작아져 1에 가깝고, 큰 음수에서는 분모가 커져 0에 가깝다. 어떤 실수 점수를 0과 1 사이로 바꾸는 규칙이다. 이 범위에 들어간다는 사실만으로 실제 확률을 정확히 예측한다는 뜻은 아니다.

$\sigma'(x)=\sigma(x)(1-\sigma(x))$이므로 $x=0$에서 기울기는 $1/4$다. 출력이 0이나 1에 가까우면 둘 중 한 인수가 작아져 기울기도 작아진다.

소프트맥스는 여러 점수를 함께 비율로 바꾼다. 점수가 $(0,\ln2)$이면 지수는 $(1,2)$, 합은 3이므로 결과는 $(1/3,2/3)$이다. 한 점수를 올리면 분모도 바뀌므로 다른 출력도 영향을 받는다. 그 때문에 미분은 숫자 하나가 아닌 야코비안 행렬이다.

**본문으로 이어 읽기:** 모든 점수에서 같은 수를 빼도 결과는 같다. 실제 계산에서는 최대 점수를 빼서 큰 지수의 넘침을 피한다. 교차엔트로피와의 결합은 뒤의 정보이론에서 의미를 배운다. 여기서는 미분이 $s_i-y_i$로 단순해지는 계산 구조를 볼 수 있다.
...
[← 이전: 라그랑주 승수법](lagrange-multipliers.html) · [다음: 행렬 미분 →](matrix-calculus.html)
