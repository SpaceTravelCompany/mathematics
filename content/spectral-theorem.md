...
> 학습 순서 23 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 고유벡터를 기저로 쓰면 행렬이 대각화되고, 정규직교 기저는 길이와 각도 계산도 보존한다.

$A=\left(\begin{smallmatrix}2&1\\1&2\end{smallmatrix}\right)$는 대각선 건너편 값들이 같아 대칭행렬이다. $(1,1)$은 3배, $(1,-1)$은 1배가 된다. 두 벡터의 내적은 0이므로 수직이다. 각각 $\sqrt2$로 나누면 길이가 1인 고유벡터 두 개가 된다.

이 벡터들을 열로 모은 $Q$는 $Q^TQ=I$를 만족한다. 그래서 $A=Q\operatorname{diag}(3,1)Q^T$는 ‘그 기준으로 좌표를 읽기 → 각각 3배와 1배 → 원래 좌표로 돌아오기’다. $\operatorname{diag}(3,1)$은 대각선에 3과 1을 놓고 나머지는 0인 행렬이다.

$x^TAx$는 행렬을 사이에 넣은 내적이며 결과는 숫자다. 새 좌표 $y=Q^Tx$로 바꾸면 $3y_1^2+y_2^2$가 된다. 섞인 항이 사라져 값의 부호와 등고선의 모양을 쉽게 읽을 수 있다.

**증명으로 이어 읽기:** 서로 다른 고유값의 벡터 $u,v$에 대해 대칭성으로 $u^TAv=(Au)^Tv$다. 양쪽에 고유값을 넣으면 $(\lambda_v-\lambda_u)u^Tv=0$이고, 고유값이 다르므로 $u^Tv=0$이다. 같은 고유값에 대응하는 공간 안에서는 그람-슈미트로 직교 기저를 고를 수 있다.
...
- **[양정치 행렬](positive-definite.html)** : 이차형식의 부호 판정은 양정치 행렬의 정의와 직접 연결된다.
- **[기저 변환](change-of-basis.html)** : 직교 대각화는 기저변환의 특수한 경우로, 정규직교기저 간의 변환이다.

---

[← 이전: 기저 변환](change-of-basis.html) · [다음: 양정치 행렬 →](positive-definite.html)
