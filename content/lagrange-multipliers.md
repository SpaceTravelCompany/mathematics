...
> 학습 순서 39 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 제약은 움직일 수 있는 위치를 제한하고, 그래디언트는 같은 값의 곡선에 수직이다.

$x+y=2$를 지키면서 $f=x^2+y^2$를 최소화하자. 먼저 아는 방법으로 $y=2-x$를 넣으면

$$f=x^2+(2-x)^2=2(x-1)^2+2.$$

따라서 $x=y=1$에서 최솟값은 2다. 승수법은 이 대입이 복잡해지는 상황에서도 쓸 수 있도록 만든다. 제약함수를 $g=x+y$라 두고 $\nabla f=\lambda\nabla g$를 쓰면 $(2x,2y)=\lambda(1,1)$이므로 $x=y$다. 제약식 $x+y=2$를 함께 써서 같은 해를 얻는다.

왜 평행해야 할까? 허용된 곡선을 따라 더 낮아질 수 있는 접선 방향이 남아 있다면 최솟값이 아니다. 최적점에서는 그 접선으로의 변화율이 0이어야 하므로 목적함수와 제약함수의 그래디언트가 같은 법선 방향을 가진다.

**증명으로 이어 읽기:** $\lambda$는 두 그래디언트의 배수 관계를 맞추는 새 미지수다. 이 방정식은 일반적으로 후보를 주므로 값을 비교해야 한다. 제약의 그래디언트가 0이 되는 특이점이나 영역의 경계는 이 조건에서 누락될 수 있어 따로 다룬다.
...
[← 이전: 극값·안장점](extrema-saddle.html) · [다음: 시그모이드·소프트맥스 미분 →](sigmoid-softmax.html)
