...
> 학습 순서 58 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 대수의 법칙은 평균이 어디로 가는지 알려주고, 중심극한정리는 그 주변에서 얼마나 흔들리는지 알려준다.

공정한 동전을 100번 던진 앞면 비율의 평균은 0.5, 표준편차는 $\sqrt{0.25/100}=0.05$다. 400번에서는 표준편차가 0.025다. 횟수를 네 배로 늘리면 평균의 흔들림은 대략 절반으로 줄어든다.

그냥 평균의 분포를 보면 점점 한 곳으로 모인다. 모양을 비교하려면 중심을 빼고 줄어든 폭으로 나눈다.

$$Z_n=\frac{\bar X_n-\mu}{\sigma/\sqrt n}.$$

기본 CLT는 독립·동일분포이고 유한한 평균과 양의 유한분산을 가진 경우에 이 $Z_n$의 분포가 표준정규분포로 가까워진다는 정리다. 원래 관측값 하나의 분포가 정규분포로 바뀐다는 뜻은 아니다. 모든 문제에서 $n=30$이면 충분하다는 규칙도 없다.

**증명으로 이어 읽기:** 특성함수 $\varphi_X(t)=\mathbb E[e^{itX}]$는 분포를 함수 하나로 기록하는 도구다. 독립인 합에서는 이 함수들이 곱해지므로 증명에 편리하다. 작은 입력에서 $1-t^2/2+o(t^2)$로 전개하고 그 곱의 극한을 구하면 정규분포의 $e^{-t^2/2}$가 나타난다. 정리 이름보다 독립성과 분산 조건을 먼저 기억하자.

---

## 직관적 설명

**중심극한정리(Central Limit Theorem, CLT)**는 아마 확률론에서 가장 놀라운 정리일 것이다: "독립이고 동일한 분포에서 얻은 값들이 유한한 평균과 양의 유한분산을 가지면, 표준화한 합이나 평균은 정규분포에 가까워진다." 원래 분포가 이산적이거나 비대칭이어도 이 조건 아래 적용할 수 있지만, 강한 의존성이나 무한분산을 가진 경우에는 그대로 적용할 수 없다.

이것이 "정규분포가 도처에 있는 이유"를 설명한다. 키처럼 여러 요인이 관여하는 양을 정규분포로 근사하는 데 이러한 직관이 쓰인다. 실제 요인들이 정리의 독립성·가법성 조건을 충족하는지까지 자동으로 보장되지는 않는다. 측정 오차가 정규분포를 따르는 것은 다양한 오차 요인이 누적되기 때문이다.
...
[← 이전: 대수의 법칙](law-large-numbers.html) · [다음: 조건부확률의 함정 →](conditional-traps.html)
