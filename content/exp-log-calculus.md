...
> 학습 순서 28 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** $\ln$은 $e^x$의 역함수이고, 합성의 변화율은 단계별 변화율을 곱한다.

$y=e^x$는 현재 값이 3인 곳에서 기울기도 3인 함수다. $y=e^{2x}$에서는 안쪽 $2x$가 두 배로 변하므로 $y'=2e^{2x}$다. 지수 앞에 2가 내려오는 이유는 거듭제곱 법칙이 아니라 연쇄법칙이다.

$y=\ln x$이면 $e^y=x$다. 양쪽을 $x$로 미분하면 $e^y y'=1$이고, $e^y=x$이므로 $y'=1/x$다. $x>0$이 필요하다. 따라서 $\ln(3x+1)$의 도함수는 $3/(3x+1)$이며 원래 함수의 정의역 $x>-1/3$에서 사용한다.

일반 밑은 $a^x=e^{x\ln a}$로 바꾸면 $(a^x)'=a^x\ln a$를 얻는다. $a=e$이면 $\ln e=1$이라서 자기 자신이 남는다.

**본문으로 이어 읽기:** 로그미분은 곱을 합으로 바꿔 미분을 단순하게 만드는 방법이다. $\ln y$를 쓸 때에는 $y>0$인지 확인하고, 부호가 바뀔 수 있는 0이 아닌 함수에는 $\ln|y|$를 사용한다. 적분 장에서는 이 결과를 뒤집어 $1/x$의 누적을 로그로 나타낸다.
...
[← 이전: 미분 법칙·연쇄법칙](differentiation-rules.html) · [다음: 적분의 의미 →](integral-meaning.html)
