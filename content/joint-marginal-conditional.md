...
> 학습 순서 56 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 조건부확률은 관심 있는 부분 안에서 확률을 다시 합계 1로 맞춘다.

두 이진 변수 $X,Y$의 결합확률을 다음처럼 정하자.
...
각 칸은 두 조건이 동시에 맞는 확률이다. $Y$를 무시하고 $X=1$만 보려면 행을 더해 0.5를 얻는다. 이것이 주변화다. $Y=1$이라고 알았을 때 $X=1$일 확률은 해당 칸 0.3을 해당 열 합 0.4로 나눈 0.75다.

독립이라면 각 칸이 행 합 × 열 합이어야 한다. 위의 $(1,1)$ 칸은 $0.5\times0.4=0.2$와 다르므로 독립이 아니다. 연속형에서는 칸의 합을 적분으로 바꾸며, 한 점의 확률 대신 밀도와 구간 확률을 사용한다.

**본문으로 이어 읽기:** 다변량 정규분포의 조건부 공식도 같은 작업이다. 관측한 성분과 아직 모르는 성분을 나누고, 함께 움직이는 정도인 공분산으로 평균을 보정한다. 자세한 숫자 계산은 [가우시안 과정](gaussian-process.html)의 관측 한 개 예제에서 다시 만난다.

---

## 직관적 설명

**결합분포(joint distribution)**는 여러 확률변수의 동시적인 거동을 완전히 기술한다. 예를 들어 키(height)와 몸무게(weight)를 연속형으로 모형화하면 결합밀도를 영역에 적분하여 "키가 174~176cm이고 몸무게가 69~71kg일 확률"을 구한다. 정확한 한 쌍의 값에 대한 확률과 밀도는 다르다.

**주변분포(marginal distribution)**는 결합분포에서 일부 변수만 보는 것이다. 키와 몸무게의 결합분포에서 몸무게를 적분하여 키만의 밀도와 구간 확률을 뽑아낸 것이 주변분포다. 이름은 결합분포표의 여백(margin)에 합계를 적는 관행에서 유래했다.
...
[← 이전: 주요 분포](distributions.html) · [다음: 대수의 법칙 →](law-large-numbers.html)
