...
> 학습 순서 66 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 조건부확률과 행렬곱이다. 이 장에서는 상태분포를 **행벡터**로 쓴다.

상태가 맑음·비 두 개라고 하자. 오늘 맑으면 내일 맑을 확률 0.8, 비면 내일 맑을 확률 0.3이라면 전이행렬은

$$P=\begin{pmatrix}0.8&0.2\\0.3&0.7\end{pmatrix}.$$

오늘 분포가 $\pi=(0.6,0.4)$라면 내일 맑을 확률은 $0.6\times0.8+0.4\times0.3=0.6$이고 비도 0.4다. 즉 $\pi P=\pi$여서 이 분포는 정상분포다. 상태 자체가 멈춘다는 뜻이 아니라 전체 비율이 유지된다는 뜻이다.

마르코프 성질은 현재 상태를 알고 나면 더 오래된 이력이 다음 상태에 추가 정보를 주지 않는다는 가정이다. 어떤 현실 문제에서 이 가정이 맞는지는 상태에 무엇을 포함했는지에 달려 있다.

**증명으로 이어 읽기:** 정상분포가 있다고 반드시 모든 시작분포가 거기로 수렴하는 것은 아니다. 항상 상태를 번갈아 바꾸는 $\left(\begin{smallmatrix}0&1\\1&0\end{smallmatrix}\right)$는 정상분포 $(1/2,1/2)$가 있어도 한 상태에서 시작하면 계속 왕복한다. 유한 상태에서 기약성과 비주기성 같은 조건이 수렴을 보장한다.
...
[← 이전: 회귀분석](regression-analysis.html) · [다음: 마르코프 체인·MCMC →](mcmc.html)
