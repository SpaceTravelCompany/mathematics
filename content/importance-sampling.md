...
> 학습 순서 70 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 표본평균은 실제로 표본을 뽑은 분포의 평균을 추정한다. 다른 분포의 평균이 필요하면 비중을 보정해야 한다.

목표분포 $p$에서 A와 B의 확률이 0.9, 0.1이고, 샘플링 분포 $q$에서는 각각 0.5라고 하자. $f(A)=0,f(B)=10$이면 목표 기댓값은 1이다. $q$에서 뽑은 값을 그냥 평균하면 5가 되어 잘못된 답을 얻는다.

가중치를 $p/q$로 정하면 A는 1.8, B는 0.2다. 보정한 기대값은 $0.5\times0\times1.8+0.5\times10\times0.2=1$이 된다. ‘더 자주 뽑힌 결과는 가볍게, 덜 뽑힌 결과는 무겁게’ 세는 방식이다. 필요한 영역에서 $q=0$이면 그 영역을 관측할 수 없어 이런 보정도 불가능하다.

재파라미터화는 다른 아이디어다. $X\sim\mathcal N(\mu,\sigma^2)$를 $X=\mu+\sigma\epsilon$, $\epsilon\sim\mathcal N(0,1)$로 만든다. 무작위 입력 $\epsilon$을 고정한 채 $\mu,\sigma$의 영향을 연쇄법칙으로 계산할 수 있다. 예를 들어 $(\mu+\sigma\epsilon)^2$의 $\mu$ 미분은 $2(\mu+\sigma\epsilon)$이다.

**본문으로 이어 읽기:** 일반 중요도 추정량의 비편향성과, 가중치 합으로 다시 나눈 자기정규화 추정량의 성질을 구분하자. 후자는 유한 표본에서 보통 편향이 있다. 가중치가 극단적으로 커지면 분산이 매우 커질 수 있다. 재파라미터 미분에서도 미분과 기댓값을 교환할 조건이 필요하다.
...
[← 이전: 마르코프 결정과정](mdp.html) · [다음: 가우시안 과정 →](gaussian-process.html)
