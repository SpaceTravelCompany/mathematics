...
> 학습 순서 26 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 함수의 기울기는 출력 변화량을 입력 변화량으로 나눈 값이다.

$f(x)=x^2$에서 $x=2$를 조금 움직여 보자. $2.1$까지 움직이면 평균 기울기는 $(2.1^2-2^2)/0.1=4.1$이다. $2.01$까지 움직이면 4.01이다. 이동폭을 $h$라고 적으면

$$\frac{(2+h)^2-2^2}{h}=\frac{4h+h^2}{h}=4+h\quad(h\ne0).$$

$h$를 0에 가까이 보내면 이 비율은 4에 가까워진다. 이 값이 $x=2$에서의 미분계수 $f'(2)$다. 처음부터 $h=0$을 넣은 것이 아니라, 0이 아닌 $h$에서 계산한 결과의 극한을 구했다. 어떤 위치 $x$에서도 같은 계산을 하면 $f'(x)=2x$라는 **도함수**를 얻는다.

연속은 입력을 조금 바꿀 때 출력도 그 점의 값에 가까이 간다는 뜻이다. 미분가능성은 거기서 기울기까지 하나로 정해진다는 더 강한 요구다. $|x|$는 0에서 끊어지지 않지만 왼쪽 기울기 -1과 오른쪽 기울기 1이 달라 미분할 수 없다.

**증명으로 이어 읽기:** $\epsilon$은 허용하는 출력 오차, $\delta$는 그 오차 안에 넣기 위해 제한하는 입력 거리다. ‘모든 $\epsilon>0$에 대해 어떤 $\delta>0$’는 출력 오차를 아무리 작게 요구해도 그 요구를 지킬 입력 범위를 정할 수 있다는 뜻이다.
...
[← 이전: SVD](svd.html) · [다음: 미분 법칙·연쇄법칙 →](differentiation-rules.html)
