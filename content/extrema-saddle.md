...
> 학습 순서 38 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 기울기 0은 극값의 후보 조건이고, 헤시안은 그 주변의 휨을 알려준다.

$f(x,y)=x^2+y^2$는 원점에서 어느 방향으로 가도 커지므로 극소다. $-x^2-y^2$는 어느 방향으로 가도 작아지므로 극대다. $x^2-y^2$는 가로로 가면 커지고 세로로 가면 작아져 안장점이다. 세 함수 모두 원점의 그래디언트는 0이다.

따라서 ‘미분해서 0이 되는 해를 구했다’는 것은 후보를 찾았다는 뜻이다. 양정치 헤시안은 엄격한 극소, 음정치는 엄격한 극대, 양·음 고유값이 섞이면 안장을 알려준다. $x^4+y^4$와 $x^4-y^4$는 원점의 헤시안이 둘 다 0이지만 전자는 극소, 후자는 안장이다.

닫힌 구간 $[0,1]$에서 $f(x)=x$의 최솟값은 0에서, 최댓값은 1에서 나온다. 어느 곳에서도 도함수가 0이 되지 않는다. 경계는 내부의 미분 조건만으로 잡히지 않으므로 별도로 봐야 한다.

**본문으로 이어 읽기:** 국소 극소는 근처에서 가장 작다는 뜻이고, 전역 최소는 허용된 전체 영역에서 가장 작다는 뜻이다. 고차원에서 안장점이 많다는 관찰은 특정 함수군이나 모형에 대한 것이며 모든 함수의 보편 법칙은 아니다.
...
[← 이전: 테일러 전개](taylor-expansion.html) · [다음: 라그랑주 승수법 →](lagrange-multipliers.html)
