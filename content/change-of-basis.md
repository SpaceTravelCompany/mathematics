...
> 학습 순서 22 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 기저를 열로 모은 $P$는 새 좌표를 원래 좌표로 보내고, $P^{-1}$은 반대로 보낸다.

$b_1=(1,1)$, $b_2=(1,-1)$을 새 기준으로 삼자. 벡터 $v=(4,2)$를 $ab_1+bb_2$로 쓰면 $a+b=4$, $a-b=2$다. 두 식을 더해서 $a=3$, 따라서 $b=1$이다. 같은 벡터의 새 좌표는 $(3,1)$이다. 벡터가 움직인 것이 아니라 그 벡터를 설명하는 숫자가 바뀌었다.

이 기저에서 행렬을 읽는 공식이 $[A]_{\text{새 기저}}=P^{-1}AP$다. 오른쪽부터 ‘좌표 풀기 → 실제 변환 → 새 좌표로 다시 읽기’라고 읽으면 곱의 순서를 외울 필요가 없다.

새 기저를 $A$의 고유벡터로 선택하면 $Ab_i=\lambda_i b_i$다. 따라서 새 좌표의 각 성분을 $\lambda_i$배만 하면 되어 행렬의 대각선 이외 자리가 0이 된다. 이것이 대각화다.

**증명으로 이어 읽기:** 비슷한 모양의 $P^{-1}AP$와 $P^TAP$를 구분하자. 전자는 선형변환의 좌표 변경, 후자는 이차식 $x^TAx$에 $x=Py$를 넣을 때 생긴다. 기저가 정규직교하여 $P^{-1}=P^T$일 때 두 식이 같아진다.
...
- **[역행렬과 기저 변환](inverse-change-of-basis.html)** : 역행렬과 기저 변환의 입문(가역성, 여인수, 가우스-조르단 역행렬)을 다룬다. 이 문서는 그 기초 위에서 닮음 변환과 기저 변환의 심화 성질을 다룬다.
...
[← 이전: 고유값·고유벡터](eigenvalues.html) · [다음: 대칭행렬·스펙트럼 정리·이차형식 →](spectral-theorem.html)
