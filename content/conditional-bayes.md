...
> 학습 순서 52 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 사건은 결과들의 모임이고, 조건은 그중 어느 결과들만 다시 볼지 정한다.

카드 10장 중 빨간 카드가 4장이고, 빨간 카드 가운데 별 표시가 3장이라고 하자. ‘빨간 카드를 뽑았다’는 정보를 얻으면 후보는 10장에서 4장으로 줄어든다. 그 안에서 별이 있는 확률은 $3/4$다. 이것이 $P(\text{별}|\text{빨강})$이며 세로줄 뒤가 주어진 정보다.

조건부확률 $P(A|B)=P(A\cap B)/P(B)$는 전체에서 센 공통 비율을 조건 $B$의 비율로 다시 나누는 것이다. 위에서는 $(3/10)/(4/10)=3/4$다. $P(B)>0$이 필요하다.

A 주머니를 고를 확률이 $1/2$, B도 $1/2$이고 빨간 공 비율이 각각 $4/5$, $1/5$라고 하자. 빨간 공을 얻는 전체 확률은 $(1/2)(4/5)+(1/2)(1/5)=1/2$다. 빨간 공을 본 뒤 A였을 확률은 $(1/2)(4/5)/(1/2)=4/5$다. 베이즈 정리는 이렇게 ‘원인별 결과 확률’을 ‘결과를 본 뒤 원인 확률’로 바꾼다.

**증명으로 이어 읽기:** 같은 교집합을 $P(A|B)P(B)$와 $P(B|A)P(A)$ 두 방식으로 쓴 뒤 정리하면 베이즈 공식이 된다. 독립은 조건을 알아도 확률이 바뀌지 않는 것이고, 배반은 두 사건이 동시에 일어나지 않는 것이다. 서로 다른 조건이다.
...
[← 이전: 경우의 수·순열·조합](counting.html) · [다음: 확률변수·PMF/PDF →](random-variables.html)
