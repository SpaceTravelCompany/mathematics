...
> 학습 순서 69 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 마르코프 체인은 다음 상태의 확률을 주고, 기댓값은 여러 미래 결과의 가중평균이다.

로봇이 현재 칸에서 왼쪽 또는 오른쪽으로 갈 수 있다고 하자. MDP에서는 현재 상태 $s$, 고르는 행동 $a$, 다음 상태 확률 $P(s'|s,a)$, 받는 보상 $R$을 지정한다. 정책 $\pi(a|s)$는 각 상태에서 행동을 어떻게 고를지 정한 규칙이다.

할인율 $\gamma=0.9$이면 지금 받는 보상 1은 그대로 1, 다음 단계의 보상 1은 현재 계산에서 0.9, 두 단계 뒤는 0.81로 센다. 보상이 계속 1이라면 총 가치는 $1+0.9+0.9^2+\cdots=10$이다.

벨만 방정식 $V(s)=\mathbb E[R+\gamma V(S')\mid s]$는 전체 미래를 ‘바로 받는 보상 + 다음 상태부터의 가치’로 나눈다. 예를 들어 보상 2를 받고 다음 상태 가치가 각각 4와 8인 곳으로 반반 이동하면 $V(s)=2+0.9(0.5\times4+0.5\times8)=7.4$다.

**증명으로 이어 읽기:** 정책평가는 정해진 행동 규칙의 가치를 구하고, 최적화는 가능한 행동 중 가장 좋은 기대값을 고른다. 무한기간의 유한 상태·행동 모형에서 유계 보상과 $0\le\gamma<1$을 가정하면 할인 덕분에 가치 반복의 오차가 줄어든다. $\gamma=1$인 문제에는 종료 같은 추가 조건이 필요하다.
...
[← 이전: 확률미분방정식](sde.html) · [다음: 중요도 샘플링·재파라미터 트릭 →](importance-sampling.html)
