...
> 학습 순서 71 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 결합 정규분포에서는 공분산이 관측한 값과 모르는 값의 연결을 나타낸다.

어떤 함수의 두 위치 값 $f_1,f_2$가 평균 0, 각각 분산 1, 공분산 0.5인 결합 정규분포를 가진다고 하자. 잡음 없이 $f_1=2$를 관측하면 조건부 평균은 $\mathbb E[f_2|f_1=2]=0.5\times2=1$, 조건부 분산은 $1-0.5^2=0.75$다. 이웃 값을 보고 예측 중심을 옮기고 불확실성을 줄인 것이다.

가우시안 과정은 이런 규칙을 유한한 두 점뿐 아니라 어떤 유한한 위치 목록에도 일관되게 적용한다. ‘함수 위의 분포’란 함수 하나를 확정하는 대신 가능한 함수들에 대한 불확실성을 표현한다는 뜻이다. 커널 $k(x,x')$은 두 위치 값의 공분산을 정하는 함수다.

위 예에서 관측잡음 분산이 0.25이면 예측 평균은 $(0.5/1.25)\times2=0.8$, 분산은 $1-0.5^2/1.25=0.8$이다. 관측을 덜 믿으므로 평균 보정도 작고 불확실성도 더 남는다.

**본문으로 이어 읽기:** 큰 행렬 공식의 $K$는 관측 위치끼리의 공분산, $K_*$는 관측과 새 위치 사이, $K_{**}$는 새 위치끼리의 공분산이다. 위의 1, 0.5, 1을 행렬로 바꾼 계산이다. 함수값의 사후구간과 새 관측값의 예측구간은 다르다. 새 관측을 예측하면 관측잡음 분산을 추가한다.
...
이며, 평균과 공분산이 모두 해석적으로 표현된다. 닫힌 공식이 있어도 계산이 항상 저렴한 것은 아니다. 관측 $n$개의 조밀한 커널 행렬을 쓰는 기본 정확 계산은 보통 $O(n^3)$ 시간과 $O(n^2)$ 저장공간을 요구한다. 비모수적이라는 말은 학습할 설정값이 전혀 없다는 뜻도 아니다.
...
**서술:** 실수값 대칭 함수 $k$가 모든 유한 점집합에서 양반정치 그람 행렬을 만들면, 이를 공분산으로 갖는 GP를 구성할 수 있다. 머서 전개에는 추가 가정이 필요하다. 예를 들어 콤팩트한 영역의 연속 대칭 양반정치 커널과 전역 지지를 가진 유한 Borel 측도라는 표준 조건 아래 다음 고유함수 전개를 얻는다. 여기서 콤팩트는 유클리드 공간에서 닫히고 유계인 영역, 측도는 적분할 때 영역별 비중을 정하는 규칙이다.
...
### 정리 3: GP 사후 평균의 최소 평균제곱 예측오차

**서술:** 주어진 GP 모형 아래 사후 평균 $\mu_*(x)=\mathbb E[f(x)|D]$은 관측 $D$로 만드는 예측 중 모형에 대한 평균제곱오차를 최소화한다. 알려진 평균과 공분산을 사용하는 정규 관측 모형에서는 선형 예측량이며 단순 크리깅과 연결된다. 고정된 임의의 참함수에 대해 언제나 불편하다는 뜻은 아니다.
...
**예제 3 (불확실성 밴드 해석):** GP 회귀에서 고정된 한 위치의 함수값에 대한 95% 사후 신용구간(credible interval)은 $\mu_*(x)\pm1.96\sigma_*(x)$이다. 모형과 하이퍼파라미터를 고정한 조건부 정규분포의 구간이며, 함수 전체를 동시에 95% 확률로 덮는 띠는 아니다. 새 잡음 관측값의 구간에는 관측잡음 분산을 추가한다.

- 데이터 포인트 $x = 0$에서: $\sigma_*^2(0) \approx \sigma_n^2 = 0.01$ (잡음 분산 수준). 사후구간 폭 $\approx 2 \times 1.96 \times 0.1 = 0.392$.
- 데이터에서 먼 $x = 10$에서: $\sigma_*^2(10) \approx k(10,10) = 1$. 사후구간 폭 $\approx 2 \times 1.96 \times 1 = 3.92$.
...
[← 이전: 중요도 샘플링·재파라미터 트릭](importance-sampling.html) · [다음: 스코어 함수·피셔 정보·크라메르-라오 하한 →](score-function.html)
