...
> 학습 순서 64 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 검정은 관측을 비교할 기준 모형과 ‘더 극단적’이라는 기준을 먼저 정한다.

공정한 동전이라는 귀무가설 아래 네 번 모두 앞면일 확률은 $1/16$이다. ‘앞면이 지나치게 많은가’를 묻는 단측검정에서 이 관측의 p-value는 $1/16$이다. ‘어느 쪽으로든 치우쳤는가’를 묻는 양측검정에서는 모두 뒷면인 경우도 세므로 이 예의 p-value는 $2/16$이다. 같은 관측이어도 사전에 정한 질문에 따라 계산할 꼬리 영역이 달라진다.

p-value는 귀무가설 아래 관측한 검정통계량만큼 또는 더 극단적인 결과가 나올 확률이다. 관측된 데이터 딱 하나의 확률도 아니고 귀무가설이 참일 확률도 아니다. 작으면 기준 모형과 잘 맞지 않는다는 증거지만, 효과가 크다거나 원인 관계가 증명되었다는 뜻은 아니다.

유의수준 $\alpha$는 귀무가설이 맞는데 기각하는 오류를 얼마나 허용할지 정한 기준이다. 기각하지 못했다고 귀무가설이 참임을 증명한 것도 아니다. 데이터가 부족해서 차이를 구별하지 못했을 수 있다.

**본문으로 이어 읽기:** 95% 신뢰구간의 95%는 같은 절차로 새 데이터를 계속 뽑을 때 만들어지는 구간들의 포함 비율을 뜻한다. 고정된 모수가 확률적으로 움직인다는 뜻이 아니다. 신뢰구간과 검정의 대응은 같은 모형과 방식으로 만든 양측검정과 구간을 짝지을 때 성립한다.
...
p-value는 가장 널리 사용되면서도 가장 자주 오해받는 통계량이기도 하다. "p-value가 0.05 미만이므로 효과가 있다"는 흔한 해석은 **틀렸다**. p-value는 $H_0$ 아래에서 관측한 검정통계량만큼 또는 더 극단적인 값이 나올 꼬리확률이다. 관측 데이터 하나의 확률 $P(\text{데이터}|H_0)$도, 가설의 사후확률 $P(H_0|\text{데이터})$도 아니며, "효과의 크기"나 "가설이 참일 확률"을 직접 측정하지 않는다.
...
[← 이전: 몬테카를로](monte-carlo.html) · [다음: 회귀분석 →](regression-analysis.html)
