...
> 학습 순서 2 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** [수학을 읽는 법](math-language.html)의 원소 기호와 ‘모든 경우’라는 말이다.

사과와 배가 든 바구니를 $A=\{\text{사과},\text{배}\}$, 배와 귤이 든 바구니를 $B=\{\text{배},\text{귤}\}$라고 하자. 두 바구니에 있는 종류를 모두 모으면 합집합 $A\cup B=\{\text{사과},\text{배},\text{귤}\}$, 양쪽에 공통인 것만 고르면 교집합 $A\cap B=\{\text{배}\}$다. 같은 배를 두 번 적지 않는 이유는 여기서 세는 것이 과일의 개수가 아니라 종류이기 때문이다.

$\{x\mid x\in A\text{이고 }x\in B\}$에서 세로줄은 ‘다음 조건을 만족하는’이라는 뜻이다. 따라서 긴 식도 ‘양쪽에 들어 있는 $x$를 모아라’라는 한 문장이다. $\emptyset$은 빈 바구니, $\{\emptyset\}$은 빈 바구니 하나를 원소로 담은 모임이므로 서로 다르다.

논리에서는 ‘4의 배수이면 짝수’와 ‘짝수이면 4의 배수’를 구별한다. 앞 문장은 맞지만 뒤 문장은 2 때문에 틀린다. 짝수라는 조건은 4의 배수이기 위해 **필요**하지만 그것만으로 **충분**하지 않다. $P\Rightarrow Q$는 $P$가 참일 때 $Q$를 보장하는 약속이다. $P$가 거짓인 경우는 약속 위반이 아니므로 논리표에서 참으로 정한다.

**증명으로 이어 읽기:** 드 모르간 법칙은 ‘A 또는 B에 들어 있지 않다’가 ‘A에도 없고 B에도 없다’와 같다는 말이다. 귀류법에서 ‘서로소’는 분수를 더 약분할 수 없다는 뜻이다. 홀수 $2k+1$의 제곱은 $2(2k^2+2k)+1$로 홀수이므로, 제곱이 짝수라면 원래 수도 짝수다. 제곱이 3의 배수인 경우도, 3으로 나눈 나머지가 1이나 2이면 제곱의 나머지가 1이라는 사실로 설명된다. 이 두 관찰이 아래 무리수 증명의 생략된 발판이다.

---

## 직관적 설명

수학은 대상을 정의하고 그 대상 사이의 관계를 추론하는 학문이다. 이 과정에서 가장 기본이 되는 두 축이 바로 **집합(set)** 과 **논리(logic)** 다. 집합은 "어떤 조건을 만족하는 대상의 모임"을 형식화한 것이고, 논리는 "참과 거짓을 판단하는 규칙"을 다룬다. 이 책에서는 집합과 논리를 대상을 정의하고 주장을 연결하는 기본 언어로 사용한다. 주장을 읽을 때는 명제 사이의 연결뿐 아니라 ‘모든’, ‘어떤’ 같은 양화사의 범위도 중요하다. 함수(function)는 집합 사이의 대응이고, 관계(relation)는 집합의 곱집합의 부분집합이며, 수(number)조차 집합으로 구성된다. 집합과 논리는 수학의 언어 그 자체인 셈이다.
...
[← 이전: 수학을 읽는 법 — 숫자에서 수식까지](math-language.html) · [다음: 다항식·방정식·부등식 →](polynomials-equations.html)
