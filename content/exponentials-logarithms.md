...
> 학습 순서 5 / 75 · [처음부터 읽기](math-language.html)

## 작은 예제로 시작하기

**앞에서 가져올 것:** 거듭제곱과 역함수의 뜻이다. 이 장의 실수 로그에서는 밑 $a>0$, $a\ne1$, 입력 $x>0$을 사용한다.

한 시간이 지날 때마다 두 배가 되는 양이 처음에 3이면, 0·1·2·3시간 후에는 $3,6,12,24$다. $t$시간 후의 값은 $3\cdot2^t$다. $2^3=8$은 ‘2를 세 번 곱하면 8’, $\log_2 8=3$은 같은 일을 거꾸로 물어 ‘8이 되려면 지수가 얼마인가’라고 읽는다.

$2^3\cdot2^2$를 펼치면 2가 총 다섯 번 곱해지므로 $2^5$다. 그래서 지수는 곱할 때 더한다. $2^{-2}=1/2^2=1/4$, $2^{1/2}=\sqrt2$는 이 규칙을 유지하도록 확장한 값이다. 음의 지수가 음의 결과를 뜻하지는 않는다.

$\log_2(8\cdot4)=\log_2 32=5=3+2$에서 보듯, 로그는 곱을 합으로 바꾼다. 그러나 $\log(8+4)$를 $\log8+\log4$로 나눌 수는 없다. 곱셈에 관한 법칙과 덧셈에 관한 법칙을 섞지 말자.

**본문으로 이어 읽기:** $e\approx2.71828$은 미분과 잘 맞는 특별한 밑이고 $\ln x=\log_e x$다. 아래 미분 성질은 뒤의 [극한과 도함수](limits-derivatives.html), [지수·로그 미분](exp-log-calculus.html)에서 계산 과정을 배운다. 지금은 지수함수와 로그함수가 서로 입력과 출력을 되돌린다는 관계를 먼저 잡는다.
...
[← 이전: 함수](functions.html) · [다음: 삼각함수 →](trigonometric-functions.html)
